...
def extract_output_files(data, prompt_id):
    """Extract output files from job data"""
    print("Extracting output files...")
    files = []
    
    try:
        # Check if we have the right structure first
        if isinstance(data, dict) and prompt_id in data:
            ...
            print("Data doesn't contain expected prompt_id key or is not a dictionary")
...
def fetch_and_download_content(api_url, prompt_id):
    ...
    if prompt_id in history_data:
        status = history_data[prompt_id].get("status", {}).get("status", "unknown")
        print(f"Job status: {status}")
        
        if status == "success":
            # Extract output files
            files = extract_output_files(history_data, prompt_id)
